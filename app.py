...
# ==================== ฟังก์ชันคำนวณความเสี่ยง (ใช้ร่วมกันทั้งหน้าประเมินและหน้าทดสอบ) ====================
def calculate_risk(age, systolic, diastolic, bs, chronic_list, family_history_list, symptoms_list):
    ...
    if systolic >= 180 or diastolic >= 110:
        total_score += 40
        bp_emergency = True
        emergency_symptoms.append(f"BP {systolic}/{diastolic}")
    ...
    if bs < 70:
        total_score += 25
        bs_emergency = True
        emergency_symptoms.append(f"BS {bs}")
    elif bs > 250:
        total_score += 25
        bs_emergency = True
        emergency_symptoms.append(f"BS {bs}")
...
with st.sidebar:
    ...
    menu = st.radio(
        ...
        ["🏠 หน้าหลัก", "🩺 ประเมินอาการใหม่", "📋 ประวัติการประเมินของฉัน",
         "📊 สถิติส่วนตัว", "🧪 ทดสอบความแม่นยำ", "ℹ️ เกี่ยวกับ"],
        index=1
    )
    st.markdown("---")
...
if menu == "🏠 หน้าหลัก":
    ...
    with col1:
        st.markdown(f'<div class="metric-card"><h3>📋 {get_data_count()}</h3><p>การประเมินของคุณ</p></div>', unsafe_allow_html=True)
    with col2:
        st.markdown('<div class="metric-card"><h3>👤 ส่วนตัว</h3><p>ข้อมูลเป็นของคุณ</p></div>', unsafe_allow_html=True)
    with col3:
        st.markdown('<div class="metric-card"><h3>🔒 ปลอดภัย</h3><p>เก็บในเครื่องคุณ</p></div>', unsafe_allow_html=True)
    st.markdown('<h2 class="section-title">📝 วิธีใช้งาน</h2>', unsafe_allow_html=True)
    col_a, col_b, col_c = st.columns(3)
    with col_a:
        st.markdown('<div style="background: rgba(255,255,255,0.95); padding: 20px; border-radius: 12px; color: black;"><b>1. ประเมินอาการ</b><br>- กรอกข้อมูลส่วนตัว<br>- เลือกอาการที่เป็น<br>- ระบบคำนวณความเสี่ยง</div>', unsafe_allow_html=True)
    with col_b:
        st.markdown('<div style="background: rgba(255,255,255,0.95); padding: 20px; border-radius: 12px; color: black;"><b>2. บันทึกข้อมูล</b><br>- ข้อมูลจะถูกบันทึกอัตโนมัติ<br>- เก็บเป็นไฟล์ CSV<br>- เปิดดูใน Excel ได้</div>', unsafe_allow_html=True)
    with col_c:
        st.markdown('<div style="background: rgba(255,255,255,0.95); padding: 20px; border-radius: 12px; color: black;"><b>3. ดูประวัติ</b><br>- ดูผลการประเมินย้อนหลัง<br>- เปรียบเทียบผลลัพธ์<br>- ดาวน์โหลดข้อมูล</div>', unsafe_allow_html=True)
    st.markdown("""
    <div class="info-box" style="margin-top: 30px;">
        <b>💾 ข้อมูลของคุณ:</b> ระบบจะบันทึกข้อมูลการประเมินทั้งหมดลงในไฟล์ <code>assessment_data.csv</code>
        ในโฟลเดอร์เดียวกับแอปนี้<br>
...
# ==================== หน้า 2: ประเมินอาการใหม่ ====================
    ...
    with st.form("assessment_form", clear_on_submit=False):
        ...
        st.markdown('<h4 style="color: #667eea; margin-top: 15px;">🧬 ประวัติสุขภาพครอบครัว (พ่อแม่/พี่น้องสายตรง)</h4>', unsafe_allow_html=True)
        ...
        st.info("💡 เลือกอาการที่คุณกำลังเป็นอยู่ สามารถเลือกได้มากกว่า 1 อาการ")
    ...
    if submitted:
        if not name:
            ...
            st.warning("⚠️ กรุณาเลือกอาการอย่างน้อย 1 รายการ หรือตรวจสอบค่าความดัน/น้ำตาล")
        else:
            # ใช้ฟังก์ชันกลาง
...
# ==================== หน้า 5: ทดสอบความแม่นยำ (ใหม่!) ====================
elif menu == "🧪 ทดสอบความแม่นยำ":
    st.markdown('<div class="page-header"><h1>🧪 ทดสอบความแม่นยำของระบบ</h1></div>', unsafe_allow_html=True)
    st.info("💡 หน้านี้ใช้ทดสอบว่าระบบประเมินความเสี่ยงได้ตรงกับเกณฑ์ทางการแพทย์หรือไม่ โดยเปรียบเทียบกับไฟล์ `test_cases.csv`")

    if not os.path.exists(TEST_FILE):
        st.error("❌ ไม่พบไฟล์ `test_cases.csv` ในโฟลเดอร์เดียวกับแอป")
        st.markdown("""
        <div class="info-box">
            <b>📝 วิธีสร้าง:</b><br>
            1. สร้างไฟล์ <code>test_cases.csv</code> (Encoding: UTF-8)<br>
            2. คอลัมน์: name, age, gender, systolic, diastolic, bs, chronic, family_history, symptoms, <b>expected_risk</b><br>
        ...
        with st.expander("👁️ ดูข้อมูลทดสอบทั้งหมด", expanded=False):
        ...
        if st.button("🚀 เริ่มทดสอบความแม่นยำ", type="primary", use_container_width=True):
            try:
                ...
                st.error("❌ ต้องติดตั้ง scikit-learn ก่อน: `pip install scikit-learn`")
                st.stop()
            ...
            st.markdown("""
            <div class="info-box">
                <b>📖 วิธีอ่าน Confusion Matrix:</b><br>
                • <b>แนวตั้ง (ซ้าย)</b> = คำตอบที่ถูกต้องจากแพทย์/เกณฑ์<br>
                • <b>แนวนอน (บน)</b> = ผลที่ระบบทำนาย<br>
                • <b>ตัวเลขในช่องทแยง</b> = จำนวนที่ตอบถูก (ยิ่งมากยิ่งดี)<br>
                • <b>ตัวเลขนอกช่องทแยง</b> = จำนวนที่ตอบผิด
            </div>""", unsafe_allow_html=True)
            ...
            st.markdown("""
            <div class="info-box">
                <b>📖 อธิบาย Metrics:</b><br>
                • <b>Precision</b> = ในบรรดาที่ระบบบอกว่าเป็น "สูง" มีกี่ % ที่สูงจริง (ยิ่งสูง = แจ้งเตือนแม่น)<br>
                • <b>Recall (Sensitivity)</b> = ในบรรดาคนที่เป็น "สูง" จริง ระบบตรวจจับได้กี่ % (ยิ่งสูง = ไม่พลาดเคสอันตราย)<br>
                • <b>F1-Score</b> = ค่าเฉลี่ยรวมของ Precision และ Recall
            </div>""", unsafe_allow_html=True)

            # --- ตารางผลลัพธ์ทุกรายการ ---
            st.markdown('<h3 class="section-title">📋 ผลลัพธ์ทุกรายการ</h3>', unsafe_allow_html=True)
            ...
            if len(wrong_df) > 0:
                ...
                st.warning(f"⚠️ ระบบตอบผิด {len(wrong_df)} เคส จาก {len(df_res)} เคส ควรวิเคราะห์สาเหตุและปรับปรุง Logic การให้คะแนน")
            else:
                st.success("🎉 ระบบตอบถูกทุกเคส! (100% Accuracy)")

            # --- สรุปสำหรับใส่เล่มรายงาน ---
            st.markdown("---")
...
# ==================== หน้า 6: เกี่ยวกับ ====================
    ...
    <div class="info-box">
        ...
        <b>✅ อัปเดต:</b> เพิ่มตรรกะ BP/BS แบบเรียลไทม์ + ประวัติครอบครัว + หน้าทดสอบความแม่นยำ
    </div>""", unsafe_allow_html=True)
    st.markdown("""
